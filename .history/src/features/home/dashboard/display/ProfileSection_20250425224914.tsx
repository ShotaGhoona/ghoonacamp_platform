...
export const ProfileSection = () => {
  const { data: weeklyGoalsData, isLoading } = useWeeklyGoals();
        ...
        <div className="w-full flex-1 gap-5 flex">
            <div className="w-[70%] h-full bg-white rounded-lg p-5 flex flex-col">
                ...
                <div className="flex-1 flex flex-col justify-center gap-2">
                    {isLoading ? (
                        <p>読み込み中...</p>
                    ) : weeklyGoalsData?.weeklyGoals.length === 0 ? (
                        <p className="text-gray-500">今週の目標はまだ設定されていません</p>
                    ) : (
                        weeklyGoalsData?.weeklyGoals.map((goal) => (
                            <p key={goal.id} className="text-sm">{goal.content}</p>
                        ))
                    )}
                </div>
            </div>
