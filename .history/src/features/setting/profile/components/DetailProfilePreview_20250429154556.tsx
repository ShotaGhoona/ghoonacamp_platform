...
export const DetailProfilePreview = ({ formData }: DetailProfilePreviewProps) => {
  return (
    ...
    <div className="flex flex-col w-full bg-white rounded-lg p-5 gap-4">
      ...
      {/* SNS関係 */}
      <div className="flex flex-col gap-2 w-full border-gray-100 border-2 rounded-lg p-4">
        <p className="text-base font-bold text-gray-400">SNS Links</p>
        <div className="flex flex-wrap gap-2">
          {formData.instagramUrl && (
            <span key={formData.instagramUrl} className="text-base text-gray-400">#{formData.instagramUrl}</span>
          )}
        </div>
      </div>
    </div>
