...
export default function Sidebar() {
  ...
  return (
    <aside className="w-full h-screen bg-[#5D6B80] text-white shadow-right">
      ...
      <nav>
        {groups.map((g) => (
          <div key={g.title} className="border-b border-[#374559] px-[15px] py-4">
            <p className="text-base font-semibold mb-2 px-2">
              {g.title}
            </p>
            {g.links.map((l) => (
              <Link
