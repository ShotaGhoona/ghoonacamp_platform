...
const groups: { title: string; links: { href: string; label: string; icon: string }[] }[] = [
  {
    title: 'Home',
    links: [
      { href: '/', label: 'Home', icon: '/svg/sidebarIcon/home.svg' },
  ...
  {
    title: 'Member',
    links: [{ href: '/members', label: 'GhoonaCamper一覧', icon: '/svg/sidebarIcon/analysis.svg' }],
  },
];
          ...
          <div key={g.title} className="border-b border-[#374559] px-[20px] py-4">
            ...
            {g.links.map((l) => (
              <Link
                ...
                href={l.href}
                className={clsx(
                  'flex items-center gap-2 rounded py-1.5 text-sm transition-colors',
                  pathname === l.href
                    ? 'bg-primary text-white'
                    : 'hover:bg-slate-600'
                )}
              >
                <img src={l.icon} alt={l.label} className="w-[20px] h-[20px] text-white" />
                <span>{l.label}</span>
              </Link>
            ))}
          </div>
